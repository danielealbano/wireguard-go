...
- **⚠ TO VERIFY** — exact JNI signature/lifecycle for the `VpnService.protect` upcall (global ref vs.
  weak ref; thread attach/detach cost per dial vs. a cached attached thread).
- **⚠ TO VERIFY** — whether a mobile WS **client** needs custom CA/servername config or system roots
  suffice for the target deployment (drives how much TLS config must cross the UAPI boundary).
